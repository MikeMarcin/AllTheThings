...
enum DiagnosticsReportBuilder {
    static func build(
        ...
        if let update = ReleaseUpdater.lastUpdateSummary(defaults: defaults) {
            lines.append("- Last Update: \(dateString(update.completedAt))")
            lines.append("- Update Target: \(update.targetVersion) (reached: \(update.targetReached))")
            lines.append("- Update Asset Type: \(update.assetType ?? "unknown")")
            lines.append("- Update Install Method: \(update.installMethod ?? "unknown")")
            lines.append("- Update Total Duration: \(durationString(update.totalDuration))")
            lines.append("- Update Download Duration: \(durationString(update.downloadDuration))")
            lines.append("- Update Preparation Duration: \(durationString(update.preparationDuration))")
            lines.append("- Update Validation Duration: \(durationString(update.validationDuration))")
            lines.append("- Update Helper Launch Duration: \(durationString(update.helperLaunchDuration))")
            lines.append("- Update Replacement Duration: \(durationString(update.replacementDuration))")
            lines.append("- Update Cleanup Warning: \(update.cleanupWarning.map(String.init) ?? "unknown")")
        }
        lines.append("")
        ...
        lines.append("- App Search Roots Configured: \(AppSettings.appSearchRootsConfigured(defaults: defaults))")
        lines.append("- App Search Root Count: \(AppSettings.appSearchRoots(defaults: defaults).count)")
        ...
        lines.append("- Global App Search Hotkey Enabled: \(AppSettings.globalAppSearchHotKeyEnabled(defaults: defaults))")
        ...
        lines.append("- Indexed Rows: \(snapshot.stats.indexedCount)")
        lines.append("- Searchable Rows: \(snapshot.stats.searchableCount)")
        lines.append("- Optimized Rows: \(snapshot.stats.optimizedCount)")
